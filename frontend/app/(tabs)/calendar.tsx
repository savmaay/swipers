...
/**
 * Logic: Conflict only if exact time strings match
 */
      ...
      {events.length === 0 ? (
        ...
        events.map((item: any) => (
          <TouchableOpacity key={item.id} style={styles.eventRow} onPress={() => onSelect(item)}>
            ...
            <View style={[styles.eventBox, timeCounts[item.time] > 1 && styles.conflictBorder]}>
                ...
                <>
                  {events.map((ev: any) => (
                    <TouchableOpacity
                      key={ev.id}
                      onPress={() => onSelect(ev)}
                      style={[styles.eventBoxSmall, styles.eventBoxSmallSpaced, timeCounts[ev.time] > 1 && styles.conflictBorderSmall]}
                    >
